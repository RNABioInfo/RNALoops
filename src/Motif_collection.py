...
class Motif:
    def __init__(self,motif_json:dict, bgsu_json:list, main_process_logger:Logger):
        ...
        if len(self.instances):
            ...
            self.sequence_dict['bgsu_reverse'] = self.reverse_sequences(self.sequence_dict['bgsu_sequences'])
        
        if len(self.rfam_ids): #get rfam sequences, rfam_lower and rfam_upper bound are only specified for hairpin loops since they are aligned
            try:self.rfam_lower_bound=int(motif_json['rfam_lower_bound']) #so that the motif itself is always in the same relative position
            except:pass
            ...
            self.sequence_dict['rfam_reverse'] = self.reverse_sequences(self.sequence_dict['rfam_sequences'])
    ...
    def get_instances(self, bgsu:list):
        ...
        for i in range(len(bgsu)):
            ...
            if ID in self.instances:
                alignments.append(Instance('hairpin',ID,bgsu[i]['alignment'],int(bgsu[i]['num_nucleotides']),self.log))
...
class Internal(Motif):
    ...
    def get_instances(self, bgsu:list):
        ...
        for i in range(len(bgsu)):
            ID=re.split('[.]',bgsu[i]['motif_id'])[0]
            if ID in self.instances:
                alignments.append(Instance('internal',ID,bgsu[i]['alignment'],int(bgsu[i]['num_nucleotides']),self.log ,int(bgsu[i]['chainbreak'])))
        self.instances=alignments
    ...
    def get_rfam_sequences(self): #Since I went through the effort of writing down every rfam internal sequence in that file, this is where they come from.
        rfam_internals_path=os.path.join(os.path.dirname(os.path.realpath(__file__)), 'data','rfam_internals_fw.csv')
...
class Instance:
    ...
    def sequence_break_api(self, nucleotides:list) -> int|bool:
        ...
        self.log.error(' Sequence break for {nuc} [{inst}] was less than 3 or not found. Sequence has been removed, will not be considered further.'.format(nuc=self.get_nucleotide_element(nucleotides[0],0)[5:],inst=self.id))
        return False
...
def load_data_json(file_name:str, RNALoops_location:str) -> list: #load local json versions in case the server is not reachable
...
def get_api_response(call:str, attempts:int = 6) -> requests.Response:
    ...
    while i < attempts:
        ...
        if response.status_code == 200:
            ...
            i += 1
    raise ConnectionError(' Could not establish connection to BGSU API.'.format(tries=attempts))

def load_jsons(main_proc_logger:Logger, RNALoops_location:str) -> list[Hairpin | Internal]: #The load_bgsu function has a backup hl_3.81/il_3.81 in case the most up to date versions can not be fetched from the bgsu servers.
...
def update(main_process_logger:Logger, deletion_bool:bool, RNALoops_location:str) -> None:
    motifs=load_jsons(main_process_logger, RNALoops_location) #type:list['Motif|Hairpin|Internal']
    if deletion_bool:
        for mot in motifs:
...
def dupe_check(motif_list:list[Hairpin|Internal], logger:Logger)-> dict[str:str]: #reworked dupe check that not only checks for duplicate sequences but also creates a dictionary with sequence:abbreviations pairs (for managing duplicates).
...
def sort_seq_dictionaries(motif: Hairpin | Internal, seq_dict: dict[str, list[str]], looptype: str = '') -> None: #modifies the dictionary inplace, so no return needed
