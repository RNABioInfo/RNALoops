...
static std::vector<Internal_location> front_collector{};
}
...
inline bool motif_match(const Basic_Sequence<alphabet,pos_type> &seq1, const Basic_Sequence<alphabet, pos_type> &seq2, T i_seq1, T j_seq1, T i_seq2, T j_seq2){
    ...
    if (j_seq1 < seq1.size() - 2 && j_seq2 < seq2.size() - 2 && i_seq1 > 2 && i_seq2 > 2){ //seq.size() - 2 covers end of sequence, i_seq > 2 covers beginning
        ...
        if (j_seq1 < seq1.size() - 9, j_seq2 < seq2.size() - 9){
            std::set<char> Internal_fronts = check_map(Subseq1,Subseq2, motif_ali::InternalHashMap_fronts);
        ...
        if (motif_ali::front_collector.size() > 0){
            res.merge(check_for_back_internals(seq1,seq2,i_seq1,j_seq1,i_seq2,j_seq2,Subseq1,Subseq2));
        }
...
template<typename alphabet, typename pos_type, typename T>
inline std::set<char> check_for_back_internals(const Basic_Sequence<alphabet,pos_type> &seq1, const Basic_Sequence<alphabet, pos_type> &seq2, T i_seq1, T j_seq1, T i_seq2, T j_seq2, Basic_Subsequence<char,unsigned int> Subseq1,Basic_Subsequence<char, unsigned int>Subseq2){
    std::set<char> res;
    for (auto paired: motif_ali::front_collector){
        //if (paired.first.first > i_seq1 && paired.first.second > j_seq1 && paired.second.first > i_seq2 && paired.second.second > j_seq2){
        std::cout<<"Part One: " << paired.first.first << "/" << paired.first.second << "   Part two: " << i_seq1 << "/" << j_seq1 << "\n";
        if (char_rnali_basepairing_internals(seq1[paired.first.first -1], seq1[j_seq1+1]) && char_rnali_basepairing_internals(seq1[paired.first.second+1], seq1[i_seq1 - 1] && char_rnali_basepairing_internals(seq2[paired.second.first-1],seq2[i_seq2+1] && char_rnali_basepairing_internals(seq2[paired.second.second +1],seq2[i_seq2-1])))){
            if (char_rnali_basepairing_internals(seq1[paired.first.first -2], seq1[j_seq1+2]) && char_rnali_basepairing_internals(seq1[paired.first.second+2], seq1[i_seq1 - 2] && char_rnali_basepairing_internals(seq2[paired.second.first-2],seq2[i_seq2+2] && char_rnali_basepairing_internals(seq2[paired.second.second +2],seq2[i_seq2-2])))){
                    Basic_Subsequence<alphabet, pos_type> front_first {seq1,paired.first.first,paired.first.second};
                    Basic_Subsequence<alphabet, pos_type> front_second {seq2,paired.second.first,paired.second.second};
                    if (auto search_first = motif_ali::InternalHashMap.get_motif_set(front_first,Subseq1); search_first != motif_ali::InternalHashMap.dupe_end()){
                        if (auto search_second = motif_ali::InternalHashMap.get_motif_set(front_second,Subseq2); search_second != motif_ali::InternalHashMap.dupe_end()){
                            res.merge(search_first->second);
                            res.merge(search_second->second);
                        }
                    }
                }
            }
