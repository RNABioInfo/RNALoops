...
//This is an alternative motified version of the MacroState grammar, where motifs can be found even if they only have a single base pair surrounding them.

//This increases the search space slightly, but not to the same extend as allowing lonley base pairs everywhere. All grammar features and restrictions

//are the same as in the original MacroState grammar, this grammar is only unambiguous if lonely base pairs are generally not allowed. Extension by Marius Sebeke, University of Stuttgart, 2025.


grammar gra_motified_macrostate uses sig_foldrna(axiom = struct) {
  ...
  strong    = {sr(BASE, weak, BASE) with basepair} | 
              {         hairpinMOT               } |
              {         leftBMOT                 } |
              {         rightBMOT                } |
              {         iloopMOT                 } # h;
  ...
  hairpinMOT= hl   (BASE, REGION with minsize(3) with unpaired with motif_h,                                                                BASE) with basepair # h;
  leftBMOT  = bl   (BASE, REGION with maxsize(30) with unpaired with motif_b,   weak,                                                       BASE) with basepair # h;
  rightBMOT = br   (BASE,                                                       weak,   REGION with maxsize(30) with unpaired with motif_b, BASE) with basepair # h;
  iloopMOT  = il   (BASE, REGION with maxsize(30) with unpaired,                weak,   REGION with maxsize(30) with unpaired,              BASE) with basepair with_overlay motif_i # h;
}