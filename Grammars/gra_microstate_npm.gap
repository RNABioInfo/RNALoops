...
grammar gra_microstate_npm uses sig_foldrna(axiom = struct) {
  struct    = sadd(BASE with unpaired, struct) |
              cadd(dangle, struct)             |
              nil(LOC)                         # h;

  dangle    = edl (BASE with unpaired, strong, LOC               ) suchthat negative_energy |
              edr (LOC,                strong, BASE with unpaired) suchthat negative_energy | 
              edlr(BASE with unpaired, strong, BASE with unpaired) suchthat negative_energy |
              drem(LOC,                strong, LOC               ) suchthat negative_energy # h;
