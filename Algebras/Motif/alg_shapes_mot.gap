...
algebra alg_shapeX_mot implements sig_foldrna(alphabet = char, answer = shape_t) { //shapeX needs  a command line argument when calling this version, but it theoretically encompasses all levels 1-5, which level is used is specified with -q [1-5] when calling ./motshapeX
