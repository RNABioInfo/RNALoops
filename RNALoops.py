...
def get_cmdarguments() -> argparse.Namespace:
    ...
    parser.add_argument( '-l',       '-log', help = 'Set log level, if log level is set to INFO or DEBUG all predictions log their time parameter', default='WARNING', dest = 'loglevel', type =str)
...
class Process:
    def __init__(self, commandline_args:argparse.Namespace):
        ...
        self.log.info('Process created successfully. Loglevel: {log}, Algorithm: {alg}, k: {k}, subopt: {sub}, motif_source: {mot}, motif_direction: {motd}, hishape_mode: {hi}, shape_level: {s}, time: {time}, algorithm_path: {algp}/{alg}'.format(log=self.loglevel,alg=self.algorithm,k=self.kvalue,sub=self.subopt,mot=self.motif_source,motd=self.direction,hi=self.hishape_mode,s=self.shape_level,time=self.time,algp=self.algorithm_path))

        self.workers = commandline_args.workers
    ...
    def compile_algorithm(self) ->bool:
        ...
        if not compilation_info.returncode: #returncode is 0 if it worked. This catches successful successful compilations.
            self.log.error("Removing temporary files...")
    ...
    def find_filetype(self) -> tuple[str, bool]: #Finds File type based on file ending
        if self.iFile.name.split('.')[-1] == 'gz' or self.iFile.name.split('.')[-1] == 'zip':
            ...
            zipped=True
            self.log.info("File is compressed, decompressing...")
        else:
    ...
    def listener(self, q:multiprocessing.Queue): #This function has the sole write access to make writing the log mp save
...
def worker(tpl:tuple, q:multiprocessing.Queue,alg): #I have no clue why but if you put this function into the Process class it will always exit with error 'cannot pickle '_io.TextIOWrapper', just leave it here
    ...
    if not result.returncode: #double negative, this captures return_code=0, returned if subprocess.run worked
        out     = result.stdout
        err     = result.stderr
        output_obj = split_results_find_subclass(tpl[0],out,alg) 
...
def split_results_find_subclass(name:str,result:str,alg:str) -> 'ClassScore|ClassScoreClass': #Would be nice to not have the check for Class decision at the end, but would need to be able to
